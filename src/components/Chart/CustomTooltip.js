...
export const ChartCustomTooltip = ({ type, payload }) => {
  ...
  const entries = payload[0].payload.entries.map((entry, j) => (
    [
      <div className="col-3" key={`${j}-1`}>{`${moment.utc(entry.startDate).format('DD.MM.YYYY')}`}</div>,
      <div className="col-3" key={`${j}-2`}>{`${entry.distance} ${entry.unit}`}</div>,
  ...
  return (
    <TooltipBackground className="container">
      <TooltipHeader className="col-3">{moment(distanceRow.payload.month).format('MMM')}</TooltipHeader>
